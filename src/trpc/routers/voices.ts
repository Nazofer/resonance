...
const voicesRouter = createTRPCRouter({
  getAll: orgProcedure.input(
    ...
    const [custom, system] = await Promise.all([
      prisma.voice.findMany({ where: {
        variant: 'CUSTOM',
        orgId: ctx.orgId, ...searchFilter
      },
  ...
  delete: orgProcedure.input(
    ...
    const voice = await prisma.voice.findUnique({
      where: { id: input.id, orgId: ctx.orgId, variant: 'CUSTOM' },
    ...
    await prisma.voice.delete({ where: { id: input.id, orgId: ctx.orgId, variant: 'CUSTOM' } });

    if (voice.r2ObjectKey) {
      await deleteAudio(voice.r2ObjectKey).catch(noop);
    }
